...
class MSCRED(tf.keras.Model):
    ...
    def create_model(self, guassian_noise_stddev = None, use_attention = True, use_ConvLSTM = True):
        print("create model!")
        ...
        if use_ConvLSTM:
            ...
            x_dedcoded_3 = conv2d_trans_layer4(x_endcoded_4[:, config.step_max-1])
            con3 = tf.keras.layers.Concatenate(axis=3, name="Con3")([x_dedcoded_3, x_endcoded_3[:, config.step_max-1]])
            x_dedcoded_2 = conv2d_trans_layer3(con3)
            con2 = tf.keras.layers.Concatenate(axis=3, name="Con2")([x_dedcoded_2, x_endcoded_2[:, config.step_max-1]])
            x_dedcoded_1 = conv2d_trans_layer2(con2)
            con1 = tf.keras.layers.Concatenate(axis=3, name="Con1")([x_dedcoded_1, x_endcoded_1[:, config.step_max-1]])
            x_dedcoded_0 = conv2d_trans_layer1(con1)
