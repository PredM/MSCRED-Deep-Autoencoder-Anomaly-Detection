...
def main():
    ...
    full_data_set = importSignatureMatrix2('../data/matrix_data_3_test/')
    with open('../data/runTestLabels.txt', 'rb') as f:
    ...
    for run in range(num_of_run_to_import):
        ...
        for i in range(curr_run.shape[0]-s):  # num_of_examples - s
            if run == i:
                ...
                print("curr_xTestData 1: ", curr_xTrainData.shape)
                ...
                print("curr_xTestData 2: ", curr_xTrainData.shape)
                curr_xTrainData = np.concatenate((curr_xTrainData, curr_example), axis=0)
                print("curr_xTestData 3: ", curr_xTrainData.shape)
                labels_new.append(labels[run])
            print(i, " curr_xTestData shape: ", curr_xTrainData.shape)
    ...
    np.save('test_data_set_3.npy', curr_xTrainData)
    labels_new_arr = np.asarray(labels_new)
    print("labels_test_new_arr: ",labels_new_arr)
    np.save('test_data_set_3_failure_labels.npy', labels_new_arr)
