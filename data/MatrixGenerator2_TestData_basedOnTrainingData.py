...
def main():
    ...
    for i in range(example_dim): #TestRuns.shape[0]-50
        print('-----------------------------------')
        if y_labels[i] == "no_failure":
            sig_mat = generate_signature_matrix_node(run=x_features[i], numOfRun=i)
            print(i,"- sig_mat: ", sig_mat.shape)
            generated_example[i, :,:,:,:] = sig_mat
        print('-----------------------------------')
        print()
    np.save(config.path + 'sig_mat_train_epsi.npy', generated_example)
